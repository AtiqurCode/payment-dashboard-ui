    ...
    <div className="space-y-6 print:p-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex-1 min-w-0">
          <h2 className="text-xl sm:text-2xl font-bold mb-2">Invoice Details</h2>
          <div className="flex flex-wrap items-center gap-2">
            <span className="font-mono text-sm sm:text-lg break-all">{invoice.invoiceNumber}</span>
      ...
      <div className="bg-card border rounded-lg p-4 sm:p-6 space-y-4 sm:space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          <div>
            <h3 className="text-sm sm:text-base font-semibold mb-2">From</h3>
            <div className="text-xs sm:text-sm space-y-1">
              <p className="font-semibold">Your Company Name</p>
              <p className="text-muted-foreground">123 Your Street</p>
              <p className="text-muted-foreground">City, State ZIP</p>
              <p className="text-muted-foreground break-all">Email: billing@yourcompany.com</p>
            </div>
          </div>

          <div>
            <h3 className="text-sm sm:text-base font-semibold mb-2">Bill To</h3>
            <div className="text-xs sm:text-sm space-y-1">
              <p className="font-semibold break-words">{invoice.clientName}</p>
              <p className="text-muted-foreground break-all">{invoice.clientEmail}</p>
              {invoice.clientAddress && (
                <p className="text-muted-foreground break-words">{invoice.clientAddress}</p>
              )}
            </div>
          </div>
        </div>
        ...
        {/* Invoice Items */}
        <div>
          <h3 className="text-sm sm:text-base font-semibold mb-3 sm:mb-4">Items</h3>
          <div className="border rounded-lg overflow-hidden overflow-x-auto">
            <table className="w-full min-w-[400px]">
              <thead className="bg-muted">
                <tr>
                  <th className="text-left p-2 sm:p-3 font-semibold text-xs sm:text-sm">Description</th>
                  <th className="text-right p-2 sm:p-3 font-semibold text-xs sm:text-sm">Qty</th>
                  <th className="text-right p-2 sm:p-3 font-semibold text-xs sm:text-sm">Price</th>
                  <th className="text-right p-2 sm:p-3 font-semibold text-xs sm:text-sm">Total</th>
                </tr>
              </thead>
              <tbody>
                {invoice.items.map((item, index) => (
                  <tr key={index} className="border-t">
                    ...
                    <td className="p-2 sm:p-3 text-right text-xs sm:text-sm">{item.quantity}</td>
                    <td className="p-2 sm:p-3 text-right text-xs sm:text-sm">${item.price.toFixed(2)}</td>
